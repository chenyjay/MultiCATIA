# CforCATIA_64.dll VB.NET 测试程序 / VB.NET Test Program

## 仓库说明 / Repository Notice

本项目仅包含 VB.NET 测试代码，不包含 CforCATIA.dll 的 C/C++ 源码。

This repository only contains the VB.NET test code. The C/C++ source of CforCATIA.dll is not included.
...
- `Program.vb` 中已默认设置为 `1027160374@qq.com`，发布前请改成你的邮箱。
...
若提示未授权或试用期结束，请检查系统时间，或修改源码中的授权码。
...
3. Double-click `VB.NetTest.exe` to run.
...
- `Program.vb` already defaults to `1027160374@qq.com`; change it to your own contact before publishing.
...
If you see an unauthorized or trial-expired message, please check your system time or update the license code in the source.
